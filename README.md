...
2. input_path: Path to a user generated input_forecast.config file, which describes the desired forecast cycle and configuration
3. valid_yaml: Path to the config validation yaml file from a prior run of nwm-cal-mgr
4. fcst_run_name: Relative path to the folder to be created for storing inputs/outputs from running ngen in nwm-fcst-mgr (ex: 'fcst_run1')
5. --use_cold_start: Optional boolean flag to generate realization and bmi config files for a cold start period (True) or for the primary forecast period (False)
6. --use_int_ana: Optional boolean flag to generate realization and bmi config files for a hindcasting intermediate AnA run
7. --use_hindcast: Optional boolean flag to generate realization and bmi config files for a hindcast run
8. --hind_cycle: Optional integer for cycle interval (in hours) between hindcast start and current hindcast run

To generate the model realization file and BMI config files for a forecast run of ngen from an existing calibration run:
1. python -m mswm.manager build_fcst path/to/input_forecast.config path/to/Output/Validation_Run/01123000_config_valid_best.yaml fcst_run1
...
2. rb = RealizationBuilder(input_path="path/to/input_forecast.config, valid_yaml="path/to/valid_best_yaml", fcst_run_name="fcst_run1", use_cold_start=False, use_int_ana=False, use_hindcast=False)
